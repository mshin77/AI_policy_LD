...
#### Content Validity Ratio (CVR) by Question

The Content Validity Ratio (CVR) is calculated using the following formula:

$$CVR = \frac{N_e - N/2}{N/2}$$

Where:
- $N_e$ is the number of experts indicating "relevant" (ratings 4-5)
- $N$ is the total number of non-neutral responses (ratings 1-2 and 4-5)
- Neutral responses (rating 3) are excluded from the calculation

Decision Rule:
- CVR ≥ 0.80 → Retain item
- CVR < 0.80 → Revise/Remove item

```{r, eval=FALSE}
round_1_cvr <- delphi_long %>%
  group_by(Question) %>%
  summarise(
    Total_Experts = n(),
    ...
    Essential_Count = sum(Rating >= 4),       
    Not_Essential_Count = sum(Rating <= 2),    
    Neutral_Count = sum(Rating == 3),          
    
    CVR_Relevant_Total = Essential_Count + Not_Essential_Count,
    
    CVR = ifelse(CVR_Relevant_Total > 0, 
                 (Essential_Count - CVR_Relevant_Total/2) / (CVR_Relevant_Total/2), 
                 0),
    
    CVR = round(CVR, 3),
    
    CVR_Decision = ifelse(CVR >= 0.80, "Retain", "Revise/Remove"),
    
    Mean_Rating = round(mean(Rating), 2),
    SD_Rating = round(sd(Rating), 2),
    Percent_Agreement = round((Essential_Count / Total_Experts) * 100, 1),
    
    .groups = 'drop'
  )

# openxlsx::write.xlsx(cvr_df, "results/round_1_cvr.xlsx", row.names = FALSE)
```
...
```{r, eval=FALSE}
round_1_plot <- ggplot(round_1_cvr, aes(x = reorder(Question, CVR), y = CVR, fill = CVR_Decision)) +
  geom_col() +
  ...
  labs(title = "",
       ...
       y = "CVR Value",
       fill = "Decision") +
  scale_fill_manual(values = c("Retain" = "darkgreen", "Revise/Remove" = "orange")) +
  ...
  theme(
    ...
    axis.title.y       = element_text(size = 12, margin = margin(r = 12), angle = 90, vjust = 0.5),
...
round_1_plotly <- plotly::ggplotly(round_1_plot, width = 800, height = 700)

round_1_plotly <- round_1_plotly %>% plotly::layout(
 annotations = list(
    list(
      x = 0,
      y = -0.16,
      text = "Note: Red line = CVR ≥ 0.80 threshold for Round 2 inclusion",
      showarrow = FALSE,
      xref = "paper",
      yref = "paper",
...
round_1_dist <- delphi_long %>%
  mutate(Response_Label = case_when(
    Rating == 1 ~ "1-Strongly Disagree",
    Rating == 2 ~ "2-Disagree", 
    Rating == 3 ~ "3-Neutral",
    Rating == 4 ~ "4-Agree",
    Rating == 5 ~ "5-Strongly Agree"
  )) %>%
  count(Question, Response_Label) %>%
  group_by(Question) %>%
  mutate(Percentage = round(n/sum(n)*100, 1)) %>%
  mutate(Rating5_Count = case_when(
    Response_Label == "5-Strongly Agree" ~ n,
    TRUE ~ 0
  )) %>%
  group_by(Question) %>%
  mutate(Max_Rating5 = max(Rating5_Count))

round_1_dist_plot <- ggplot(round_1_dist, aes(x = reorder(Question, Max_Rating5, decreasing = FALSE), y = n, fill = Response_Label)) +
...
save(dfm, word_frequency_plot, word_frequency_remove_stopwords, word_frequency_remove_common,
     ...
     round_1_cvr, round_1_plot, round_1_plotly, 

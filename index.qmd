...
```{mermaid}
flowchart TD
    n1["Policy Documents Analysis<br/>N = 12 Documents"] --> D["Document Categorization<br/>• SLD Policies<br/>• Other Disability Policies<br/>• General AI Policies"]
    D --> E["Text Preprocessing<br/>• Tokenization<br/>• Stopword Removal<br/>• Lemmatization"]
    E --> F["Text Embeddings<br/>Sentence-BERT<br/>all-MiniLM-L6-v2"] & G["Topic Modeling<br/>Structural Topic Model"]
    F --> H["Similarity Analysis<br/>Cosine Similarity<br/>Threshold: 0.6"]
    G --> I["Topic Label Generation<br/>OpenAI's API<br/>'gpt-3.5-turbo'"]
    I --> V1["Research Team Validation<br/>Topic Labels"]
    F --> V1
    H --> P["Cross-Category Theme Comparison<br/>• SLD vs Other Disability<br/>• SLD vs General Policies"]
    P --> Q["Gap Identification<br/>• Unique SLD Themes<br/>• Missing Elements<br/>• Cross-Policy Opportunities"]
    n1 --> TP2["Text Preprocessing<br/>• Tokenization<br/>• Stopword Removal<br/>• Lemmatization"]
    TP2 --> TM2["Topic Modeling<br/>Structural Topic Model"]
    TM2 --> J["Survey Item Generation<br/>OpenAI's API<br/>'gpt-3.5-turbo'"]
    ...
    K2 --> L
    L --> STM3["Topic Modeling Refinement<br/>Structural Topic Model"]
    STM3 --> M["Final Policy Items<br/>N=36 Items"]
    M --> T["Theme Generation<br/>OpenAI's ChatGPT<br/>'gpt-4o'"]
...
### Policy Document Similarity Heatmap
...
cross_policy <- ld_similarities %>%
    ...
    arrange(doc_numeric_id) %>%
    mutate(gap_type = "Cross-Policy_Document")
...
missing_from_ld %>%
    arrange(doc_numeric_id) %>%
    select(other_doc_name, min_similarity, max_similarity, match_sld, other_category, gap_type) %>%
    DT::datatable(
        options = list(
            dom = 'Bfrtip',
            buttons = c('copy', 'excel', 'pdf'),
...
all_labeled_topics <- all_labeled_topics %>%
  mutate(
    topic = as.numeric(topic),  # Ensure topic is numeric
...
```{r, echo=FALSE}
topic_similarity_heatmap_plotly
```
...
cross_policy_topics <- topic_similarities %>%
  ...
  arrange(topic_numeric_id) %>%
  mutate(gap_type = "Cross-Policy_Learning")
```
...
all_labeled_topics %>%
  arrange(category, topic) %>%
  mutate(topics = topic_label_with_gamma,
         category = factor(category, levels = c("SLD", "Other Disability", "General"))) %>%
  select(-topic_label_with_gamma) %>%
  group_by(category, topics) %>%
  summarise(
...
```{r, echo=FALSE}
save(document_summary,
     single_tbl_ft, multi_tbl_ft,
     all_labeled_topics,
     ld_similarities, 
     similarity_stats, LD_Unique, missing_from_ld, cross_policy,
     similarity_heatmap, similarity_heatmap_plotly,
     dfm_ld, dfm_disability, dfm_general, topic_model_ld, topic_model_disability, topic_model_general,
     topic_similarities, topic_similarity_stats, topic_similarity_heatmap, topic_similarity_heatmap_plotly,
     LD_Unique_topics, missing_from_ld_topics, cross_policy_topics, 
